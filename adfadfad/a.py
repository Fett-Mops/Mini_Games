...
class game_of_live:
    def __init__(self) -> None:
        self.cords = {}
        self.row = self.column = 24
        self.col ={True:(255, 250, 129),False:(218, 184, 148),'bg' : (72, 181, 163)}
        ...
        self.bits =  [[[]for _ in range(self.column)] for _ in range(self.row)]    
        self.update = []
        self.draw('setup')
        ...
        if spec == 'setup':
            ...
            for j in range(self.row):
                for i in range(self.column):
                    bit = pg.draw.rect(self.screen,self.col[False],(10+int(i)*25, 10+int(j)*25, 20, 20))
